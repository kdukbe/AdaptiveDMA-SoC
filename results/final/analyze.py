...
def main() -> None:
    ...
    adaptive = [row for row in samples if row["mode"] == "ADAPTIVE"]
    transitions = [
    ...
    if len(transitions) != reported_transitions or up == 0 or down == 0:
        raise ValueError("Adaptive transition count does not reconcile")
    ...
    if phase_dma:
        ...
        for summary_row in summary_reported:
            ...
            for field in (
                "commands", "dma_cycles", "r_wait", "w_wait", "r_windows",
                "w_windows",
            ):
